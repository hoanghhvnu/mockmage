 ...
 * Time: 22:08
 */
class Training_Album_Adminhtml_AlbumController extends Mage_Adminhtml_Controller_action
{

    protected function _initAction() {
        $this->loadLayout()
            ->_setActiveMenu('album/items')
    ...
    public function editAction() {
        ...
        $model  = Mage::getModel('album/album')->load($id);
        ...
        if ($model->getId() || $id == 0) {
            ...
            $this->loadLayout();
            $this->_setActiveMenu('album/items');
            ...
            $this->_addContent($this->getLayout()->createBlock('album/adminhtml_album_edit'))
                ->_addLeft($this->getLayout()->createBlock('album/adminhtml_album_edit_tabs'));
    ...
    public function saveAction() {
        if ($data = $this->getRequest()->getPost()) {
            ...
            $model = Mage::getModel('album/album');
            $model->setData($data)
        ...
        if(!is_array($albumIds)) {
            ...
            try {
                foreach ($albumIds as $albumId) {
                    $album = Mage::getModel('album/album')->load($albumId);
                    $album->delete();
                }
